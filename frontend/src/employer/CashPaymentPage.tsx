    ...
    <motion.div className="p-6 max-w-4xl" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Pay guards in cash with OTP confirmation</p>
        </div>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: 'white', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>{['Guard', 'Site', 'Amount', 'Date', 'Status'].map(h => <th key={h} className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">{h}</th>)}</tr></thead>
            <tbody>
              {payments.map(p => (
            ...
            <motion.div className="w-full max-w-md rounded-2xl bg-white p-6" initial={{ scale: 0.95 }} animate={{ scale: 1 }} onClick={e => e.stopPropagation()}>
              ...
              {step === 'otp' && (
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-400 mb-4">Ask the guard for the code to confirm they received {inr(Number(amount))} <span className="font-mono font-bold text-blue-600">(demo: 123456)</span></p>
