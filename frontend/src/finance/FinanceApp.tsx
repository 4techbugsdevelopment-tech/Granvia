...
export default function FinanceApp({ onLogout }: { onLogout: () => void }) {
  ...
  const decide = async (row: WithdrawalRow, decision: 'approve' | 'reject') => {
    ...
    try { await decideWithdrawal(row.id, decision, reason || undefined); await load(); setNotice(decision === 'approve' ? 'Withdrawal approved successfully.' : 'Withdrawal rejected successfully.'); }
  ...
  const complete = async (row: WithdrawalRow) => {
    ...
    if (!reference) return;
    setBusy(row.id); setError(null); setNotice(null);
    try { await completeWithdrawal(row.id, reference); await load(); setNotice('Payout recorded successfully.'); }
    catch (cause: any) { setError(cause.message); }
    finally { setBusy(null); }
  };
